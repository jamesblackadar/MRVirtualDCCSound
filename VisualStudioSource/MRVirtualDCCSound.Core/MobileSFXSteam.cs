...
{
    /// <summary>
    /// Conventional Steam Engine Sound
    /// </summary>
    public class MobileSFXSteam : MobileSFXBase, IMobileSFXBase
    {
//TODO, we could optionally read in json config files per loco that might define the decoders speedsteps
//vstart,vmid,vend or full 28 step map. 
        public MobileSFXSteam(MobileSFXBase item, ILogger logger) : base(item,logger)
        {
        }
        
        public override void SetSpeed(byte speed)
        {
            if (_soundMotion == null || _soundIdle == null) return;
            var newSpeed = new SpeedState(speed, DirectionReversed, _lastSpeed);
            if (newSpeed.State == SpeedState.Mode.NoChange) return;

            if (semaphoreLock.WaitOne(100))
            {
                try
                {
